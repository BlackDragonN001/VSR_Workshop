...
function GetSpawnpointForTeam(Team, allyTeam, FRIENDLY_SPAWNPOINT_MAX_ALLY, FRIENDLY_SPAWNPOINT_MIN_ENEMY, RANDOM_SPAWNPOINT_MIN_ENEMY)
	...
	if(not IsTeamplayOn())
	then
		local pAllySpawnPointInfo = GetAllSpawnpoints(allyTeam);
		...
		for i = 1, count
		do
			if((pAllySpawnPointInfo[i].DistanceToClosestSameTeam >= FRIENDLY_SPAWNPOINT_MAX_ALLY) and
