...
function _STSV.CreateVehicles(Team, TeamRace, VariantName, Bitmask, Where)
	...
	do
		if bit32.band(Bitmask, bit32.lshift (1, i-1)) > 0 then
			...
			local BaseODF = TeamRace .. StartingVehicles.s_StartingVehicleList[i]:sub(2);
			local NewODF = BaseODF .. VariantName;

			if not DoesODFExist(NewODF) then
				NewODF = BaseODF;
			end
